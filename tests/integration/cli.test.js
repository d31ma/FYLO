...
describe('CLI', () => {
    test('version control commands create isolated branches and commit history', async () => {
        ...
        const initialCommit = await run(
            ['src/cli/index.js', 'commit', '-m', 'initial main snapshot', '--root', root, '--json'],
            repo
        )
        ...
        const checkoutFeature = await run(
            ['src/cli/index.js', 'checkout', '-b', 'feature/docs', '--root', root, '--json'],
            repo
        )
        ...
        const featureCommit = await run(
            ['src/cli/index.js', 'commit', '-m', 'feature snapshot', '--root', root, '--json'],
            repo
        )
        ...
        const featureSelect = await run(
            ['src/cli/index.js', 'sql', 'SELECT * FROM vc-posts', '--root', root],
            repo
        )
        ...
        const branchList = await run(['src/cli/index.js', 'branch', '--root', root, '--json'], repo)
        ...
        const log = await run(['src/cli/index.js', 'log', '--root', root, '--json'], repo)
        ...
        const cleanStatus = await run(
            ['src/cli/index.js', 'status', '--root', root, '--json'],
            repo
        )
        ...
        const dirtyDiff = await run(['src/cli/index.js', 'diff', '--root', root, '--json'], repo)
        ...
        const checkoutMain = await run(
            ['src/cli/index.js', 'checkout', 'main', '--root', root, '--json'],
            repo
        )
        ...
        const duplicate = await run(
            ['src/cli/index.js', 'checkout', '-b', 'feature/docs', '--root', root],
            repo
        )
    ...
    test('version control merge handles clean merges and reports conflicts', async () => {
        ...
        await run(['src/cli/index.js', 'commit', '-m', 'base', '--root', root, '--json'], repo)

        const checkoutFeature = await run(
            ['src/cli/index.js', 'checkout', '-b', 'feature/merge', '--root', root, '--json'],
            repo
        )
        expect(checkoutFeature.exitCode).toBe(0)
        const feature = new Fylo(root, manualVersioning)
        ...
        const featureCommit = await run(
            ['src/cli/index.js', 'commit', '-m', 'feature work', '--root', root, '--json'],
            repo
        )
        expect(featureCommit.exitCode).toBe(0)
        const featureCommitResult = JSON.parse(featureCommit.stdout)
        ...
        const mainCommit = await run(
            ['src/cli/index.js', 'commit', '-m', 'main work', '--root', root, '--json'],
            repo
        )
        ...
        const mergedSelect = await run(
            ['src/cli/index.js', 'sql', 'SELECT * FROM merge-posts', '--root', root],
            repo
        )
        ...
        await run(
            ['src/cli/index.js', 'commit', '-m', 'conflict base', '--root', conflictRoot],
            repo
        )
        await run(
            ['src/cli/index.js', 'checkout', '-b', 'feature/conflict', '--root', conflictRoot],
            repo
        )
        ...
        await run(
            ['src/cli/index.js', 'commit', '-m', 'feature edit', '--root', conflictRoot],
            repo
        )
        await run(['src/cli/index.js', 'checkout', 'main', '--root', conflictRoot], repo)
        ...
        await run(['src/cli/index.js', 'commit', '-m', 'main edit', '--root', conflictRoot], repo)

        const conflictMerge = await run(
            ['src/cli/index.js', 'merge', 'feature/conflict', '--root', conflictRoot, '--json'],
            repo
        )
    ...
    test('the CLI supports SQL and richer admin commands', async () => {
        const repo = process.cwd()
        const root = await createRoot('fylo-cli-')

        const create = await run(
            ['src/cli/index.js', 'sql', 'CREATE TABLE cli-posts', '--root', root],
            repo
        )
        ...
        const rebuild = await run(
            ['src/cli/index.js', 'rebuild', 'cli-posts', '--root', root, '--json'],
            repo
        )
        ...
        const select = await run(
            ['src/cli/index.js', 'sql', 'SELECT * FROM cli-posts', '--root', root],
            repo
        )
        ...
        const inspect = await run(
            ['src/cli/index.js', 'inspect', 'cli-posts', '--root', root, '--json'],
            repo
        )
        ...
        const deleted = await run(
            ['src/cli/index.js', 'deleted', 'cli-posts', '--root', root, '--json'],
            repo
        )
        ...
        const restore = await run(
            ['src/cli/index.js', 'restore', 'cli-posts', cliDocId, '--root', root, '--json'],
            repo
        )
        ...
        const inspectWorm = await run(
            ['src/cli/index.js', 'inspect', 'cli-worm', '--root', root, '--worm', '--json'],
            repo
        )
        ...
        const getHistorical = await run(
            ['src/cli/index.js', 'get', 'cli-worm', originalId, '--root', root, '--worm', '--json'],
            repo
        )
