...
export function fyloTarball() {
    ...
    const pack = run(
        npmCommand,
        ['pack', '--json', '--ignore-scripts', '--pack-destination', packageTempRoot],
        {
            cwd: repoRoot
        }
    )
    ...
    const filename =
        Array.isArray(metadata) && typeof metadata[0]?.filename === 'string'
            ? metadata[0].filename
            : undefined
    if (!filename) {
...
export async function createFyloConsumer() {
    ...
    const install = run(
        'bun',
        ['add', '--backend=copyfile', '--cache-dir', path.join(root, '.bun-cache'), tarball],
        { cwd: root }
    )
