...
// Instances that have already kicked off boot(). A WeakSet (not a reactive
// field) so the template's one-time boot trigger doesn't itself cause renders.
// We avoid the `@onMount` decorator here: the Tachyon compiler mis-scopes the
// auto-imported `onMount` helper on a component this large, so the decorator
// form throws "onMount is not defined" at build. Triggering boot from a hidden
// template interpolation is the decorator-free equivalent.
...
export default class {
    ...
    rawUrl = ''
    rawSize = 0
    /** @type {'' | 'image' | 'video' | 'audio' | 'frame' | 'text' | 'other'} */
    previewKind = ''
    ...
    // Called from a hidden template interpolation on every render; fires boot()
    // exactly once, in the browser only (matching the old @onMount semantics).
    ensureBooted() {
        if (this.tac?.isBrowser && !BOOTED.has(this)) {
            BOOTED.add(this)
    ...
    resizeColumnKey(event, key) {
        const col = event.currentTarget.closest('.explorer-col')
        if (!col) return
    ...
    resizePaneKey(event, key) {
        const body = event.currentTarget.closest('.explorer-body')
        if (!body) return
