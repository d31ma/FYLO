...
export default class extends Tac {
  ...
  sqlBody(lang, cmt) {
    switch (lang) {
      ...
      default:
        return [call(lang, { op: 'executeSQL', sql: 'SELECT * FROM users WHERE active = true' })]
    }
  }
  ...
  schemaCode() {
    return [
      ...
      '  rules.json             ← optional row-level security rules',
    ].join('\n')
  }

  securityCode() {
    return [
      '# Encryption & RLS are declared on disk — they apply to every client.',
      '',
