...
const validateProperty = (schema, data, schemaKey, path) => {
    ...
    if (Array.isArray(schemaValue)) {
        ...
        if (!Array.isArray(value))
            throw new CHEXError(`Type mismatch for '${fullPath}': expected an array`)
        ...
        if (typeof item === 'string') {
            ...
            value.forEach((element, index) => {
                if (!isObject(element))
                    throw new CHEXError(
                        `Type mismatch for '${fullPath}[${index}]': expected an object`
                    )
    ...
    if (isObject(schemaValue)) {
        if (!defined) return nullable ? undefined : rejectMissing(fullPath)
        if (!isObject(value))
            throw new CHEXError(`Type mismatch for '${fullPath}': expected an object`)
...
export const validate = (schema, data) => {
    ...
    if (Object.keys(schema).length === 0)
        throw new CHEXError('Schema must define at least one property')
